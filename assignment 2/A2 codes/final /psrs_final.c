...
#define MAXNUMBER 1000   /* maximum number for random array generation */
...
int main(int argc, char *argv[])
{
    ...
    int data_size = 3000000; // Size of data to sort
    
    // Generate the random array
    int* random_array_seq = NULL;
    if (taskid == MASTER) {
        random_array_seq = create_array(data_size);
    }
    ...
    for (int i = 0; i < numtasks; i++) {
        int idx = i * actual_local_size / numtasks;
        if (idx < actual_local_size) {
            samples[i] = local_array[idx];
        } else {
            samples[i] = local_array[actual_local_size - 1];
        }
    }
    ...
    for (int i = 0; i < actual_local_size; i++) {
        while (current_partition < numtasks - 1 && local_array[i] > pivots[current_partition]) {
            current_partition++;
        }
    ...
    int pos = 0;
    for (int i = 0; i < numtasks; i++) {
        memcpy(&send_buffer[pos], partitions[i], partition_sizes[i] * sizeof(int));
        pos += partition_sizes[i];
    }

    int *recv_buffer = (int *)malloc(total_recv * sizeof(int));
    MPI_Alltoallv(send_buffer, partition_sizes, send_offsets, MPI_INT, recv_buffer, recv_counts, recv_offsets, MPI_INT, MPI_COMM_WORLD);
    ...
    if (taskid == MASTER) {
        ...
        gather_counts = (int *)malloc(numtasks * sizeof(int));
        gather_offsets = (int *)malloc(numtasks * sizeof(int));
    }
    MPI_Gather(&total_recv, 1, MPI_INT, gather_counts, 1, MPI_INT, MASTER, MPI_COMM_WORLD);
    if (taskid == MASTER) {
        gather_offsets[0] = 0;
        for (int i = 1; i < numtasks; i++) {
    ...
    MPI_Gatherv(recv_buffer, total_recv, MPI_INT, final_sorted, gather_counts, gather_offsets, MPI_INT, MASTER, MPI_COMM_WORLD);
    ...
    if (taskid == MASTER) {
        ...
        // Optionally, print the final sorted array
        // printf("\nFinal sorted array:\n");
        // printArray(final_sorted, data_size, demo_mode);
    }

    // Clean up
    if (random_array_seq) free(random_array_seq);
    free(local_array);
    free(samples);
    ...
    free(send_buffer);
    free(recv_buffer);
    if (final_sorted) free(final_sorted);
    if (gather_counts) free(gather_counts);
    if (gather_offsets) free(gather_offsets);
